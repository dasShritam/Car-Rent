...
export function EditCar() {
  ...
  async function handleSubmit(e) {
    ...
    const data = await fetch('http://localhost:3005/cars/' + values.id, {
      method: 'PUT',
      headers: {
  ...
  return (
    <section className={styles['car-edit-container']}>
      <div className={styles['car-edit-container-fluid']}>
        <h1 className={styles['title-edit']}>
          Edit {values.make} {values.model}
        </h1>
        ...
        <form onSubmit={handleSubmit} className={styles['car-info-container']}>
          ...
          <p>
            <button className={styles['btn-edit-car']}>Edit car</button>
          </p>
        </form>
      </div>
    </section>
...
function validateForm(values) {
  ...
  if (!values.make) {
    ...
    validation.errors.make = 'Please enter car make';
  }
